...
6. **Pages.** Run `adventure guide <pages dir> --theme <name>`, which writes `guide.css` and the theme's `theme.css` side by side. For a simple day, fill `templates/day.html` with `render.render_day(day, template, out, theme=theme.load("<name>"))`, which carries the theme inside the page. For a hand written page, link both stylesheets, theme first: a page with only `guide.css` has no colours and no typefaces at all. For a richer day, write the page by hand with the same classes: an opener page with the day's lead poster, then one page per part of the day with the poster, the timed rows, a short history note and pasted ephemera with credits.
