...
4. **Posters.** With `GEMINI_API_KEY` set and the traveller's OK to spend on it:
   `adventure poster --style <style.json> --concept "<concept>" --reference-note "<what the photo is for>" --ref <ref.jpg> --out trips/<id>/art/dayN/raw --name <stop>`
   Without a key, make a card instead and let the traveller paste it into Google Flow:
   `adventure flow-card --style <style.json> --concept "<concept>" --ref <ref.jpg> --out trips/<id>/art/dayN/cards --name <stop>`
