...
def test_prompt_command_builds_from_a_named_theme(capsys):
    assert cli.main(["prompt", "--theme", "kyoto-woodblock", "--concept", "A fox shrine at dusk."]) == 0
    out = capsys.readouterr().out
    assert "Concept: A fox shrine at dusk." in out
    assert "ukiyo-e" in out
